...
g_NMDAR,n_ip3r,input_pattern,f_input,n_inputs = input("Input gNMDAR (pS), no. of IP3R, input_type, input frequency (Hz), and no. of inputs: ").split(' ')

if input_pattern not in ['rdp','stdp']:
    print('input_type can only be "rdp" or "stdp"')
    raise SystemExit

if input_pattern == 'stdp': n_bap, tdiff = input("Input no. of BAP per input and spike timing difference (ms): ").split(' ')
...
def spine_model(x,t):
    ...
    mv_eq = ((1.0/(1 + expit(-(u-um)/kmv))) - mv)/tau_mv
    hv_eq = ((1.0/(1 + expit(-(u-uh)/khv))) - hv)/tau_hv
    I_vgcc = -0.001 * Nav * 3.2e-19 * g_vgcc * (mv**2) * hv * 0.078 * u * (ca - ca_ext*expit(-0.078*u))/(1 - expit(-0.078*u))
...
print('Initial IP3 open probability = ', open_prob[-1])
...
print('Initial IP3 open probability= ', open_prob_ad[-1])
...
open_prob_ad = [((ip3/(ip3+d1))*(ca/(ca + d5))*h)**3 for ip3,ca,h in zip(sol_ad[:,2],sol_ad[:,-1],sol_ad[:,-3])]
nmdar_flux_ad = [-(g_N_Ca/Vspine) * (I_N(s,u,i)/(g_N*(u - E_N))) * 0.078 * u * (ca - ca_ext*np.exp(-0.078*u))/(1 - np.exp(-0.078*u)) for i,u,ca in zip(t,sol_ad[:,-5],sol_ad[:,-1])]
...
plt.subplot(4,2,1)
plt.plot(t,sol_ad[:,-1],'b-',label='Ca2+ with AD',linewidth=2.2)
plt.plot(t,sol[:,-1],'r-',label='Ca2+ without AD',linewidth=2.2)
plt.legend(loc='best')
plt.ylabel('Concentration ($\mu$M)',size=10)

plt.subplot(4,2,3)
plt.plot(t,[np.sum(sol_ad[k,35:43]) for k in range(len(t))],'b--',label='aCaM with AD',linewidth=2.2)
plt.plot(t,[np.sum(sol[k,35:43]) for k in range(len(t))],'r--',label='aCaM without AD',linewidth=2.2)
plt.legend(loc='best')
plt.ylabel('Concentration ($\mu$M)',size=10)

plt.subplot(4,2,2)
plt.plot(t,sol_ad[:,-5],'b-',label='Spine V with AD',linewidth=2.5)
plt.plot(t,sol[:,-5],'r-',label='Spine V without AD',linewidth=2.5)
plt.legend(loc='best')
plt.ylabel('Voltage (mV)',size=10)

plt.subplot(4,2,4)
plt.plot(t,nmdar_flux,'g-',linewidth=2.5,label='NMDAR flux')
plt.plot(t,ip3r_flux,'m-',linewidth=2.5,label='IP3R flux without AD')
plt.plot(t,ip3r_flux_ad,'b-',linewidth=2.5,label='IP3R flux with AD')
plt.legend(loc='best')
plt.ylabel('Ca influx rate ($\mu$M/s)',size=10)
 
plt.subplot(4,2,5)
plt.plot(t,sol_ad[:,-6],'b-',label='w with AD',linewidth=2.6)
plt.plot(t,sol[:,-6],'r-',label='w without AD',linewidth=2.6)
plt.legend(loc='best')
plt.ylabel('Synaptic weight change (a.u.)',size=10)
plt.xlabel('Time (s)',size=10)

plt.subplot(4,2,6)
plt.plot(t,sol[:,2],'c-',label='IP3 without AD',linewidth=2.2)
plt.plot(t,sol_ad[:,2],'b-',label='IP3 with AD',linewidth=2.2)
plt.legend(loc='best')
...
plt.subplot(4,2,7)
plt.plot(t,open_prob,'m-',linewidth=2.5,label='Open prob without AD')
plt.plot(t,open_prob_ad,'b-',linewidth=2.5,label='Open prob with AD')
plt.legend(loc='best')
plt.ylabel('Open probability of IP3',size=10)
